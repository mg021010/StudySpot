...
| 영역 (Layer) | 상황 (Context) | 대안 (Alternatives) | 선택 기준 (Criteria) | 결정 (Decision) | 결과 - 얻은 것 (Pros) / 포기한 것 (Cons) |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **엣지 임베디드** | 라즈베리파이 3에서 44.1kHz 오디오 RMS 연산 및 BLE 스캔/PIR 상태 추적을 병렬 스레드로 충돌 없이 실시간 처리해야 함. | `Python` (RPi.GPIO),<br>`Go` (고루틴 기반 동시성) | • 가비지 컬렉션(GC)으로 인한 순간 지연(Stop-the-World)이 전혀 없을 것<br>• 리눅스 커널 저수준 API(ALSA, BlueZ)에 다이렉트 바인딩 가능할 것 | **C++17** | **[얻은 것]** CPU/메모리 부하 최소화, 렉이 없는 결정론적(Deterministic) 센싱 루프 수립, 커널 API 직접 제어<br>**[포기한 것]** 수동 메모리 관리 복잡성, 크로스 컴파일 환경 구축의 번거로움 |
| **미들웨어 브릿지** | 엣지 노드의 MQTT JSON 패킷을 상시 구독하여 Firebase 실시간 DB 규격 및 토큰 인증에 맞춰 안정적으로 수송해야 함. | `C++` (노드 직접 전송),<br>`Node.js` (비동기 이벤트 루프) | • 비즈니스 로직 및 전송 규격 수정을 컴파일 없이 빠르게 반영할 수 있을 것<br>• Firebase/MQTT 생태계 활용과 폴백(Fallback) 예외 구현이 용이할 것 | **Python 3** | **[얻은 것]** 개발 시간 단축, 중계 브릿지의 뛰어난 가독성 및 유지보수성, 풍부한 연동 패키지 활용<br>**[포기한 것]** C++ 대비 실행 속도 저하 (1초 주기의 라이브 브릿지 데이터 처리 환경에서는 영향 없음) |
| **프론트엔드 대시보드** | 웹소켓으로 수신하는 공간 랭킹 정보를 1초마다 대시보드 UI 카드 정렬 및 차트에 렉 없이 동적으로 렌더링해야 함. | `React`,<br>`Next.js` (모던 프레임워크) | • 빌드/컴파일 없이 파일 더블클릭만으로 즉시 구동되는 데모 환경일 것<br>• 웹소켓 이벤트를 가볍게 DOM 제어로 처리해 FPS 드롭을 방지할 것 | **Vanilla JS** | **[얻은 것]** 제로 빌드 오버헤드, 가벼운 브라우저 초기 로딩 속도, CSS 트랜지션을 이용한 렉 없는 랭킹 스무딩 최적화<br>**[포기한 것]** 컴포넌트 재사용성과 대규모 상태 관리 확장 아키텍처 배제 |
