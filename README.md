...
* **1단계. 이 프로젝트는 [문제]를 해결합니다**
  * 사용자가 공간에 직접 가보기 전에는 알 수 없는 **실시간 소음 분위기(노트북 소음, 대화 소리 등)와 정확한 혼잡도를 사전에 진단해 주는 공간 큐레이션의 부재**를 해결합니다. 기존 서비스가 해결하지 못한 '가만히 앉아 공부하는 정지 상태의 재실자를 감지하지 못하는 오류'와 '음성 도청 등 프라이버시 침해 우려로 인해 현장 소음 상태를 분석하기 어려웠던 문제'를 동시에 해결합니다.
* **2단계. 저는 [역할]로 [핵심 구현]을 맡았습니다**
  * **[역할]**: **임베디드 엣지 컴퓨팅 시스템 구축 및 실시간 중계 파이프라인 설계**
  * **[핵심 구현]**:
    * **C++17 기반 프라이버시 보호형 엣지 연산**: 현장에서 수집되는 음성 Raw 데이터를 전송 즉시 파기하여 사생활을 보호하는 한편, 소음 분위기(Acoustic) 분류 및 BLE-PIR 융합 재실 판정 로직을 라즈베리파이 단에 최적화하여 구현했습니다.
    * **초저지연 데이터 중계망 구축**: 엣지에서 연산된 경량 데이터 패킷을 MQTT 브로커와 Firebase 실시간 DB를 연결해 새로고침 없이 1초 주기로 브라우저 대시보드 화면에 동기화하는 파이썬 중계 브릿지를 개발했습니다.
