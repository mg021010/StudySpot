...
프로젝트 설계 및 컴파일 과정에서 도출된 주요 기술적 고민과 대안별 트레이드오프 분석 내용입니다.
...
* **얻은 것 (Pros)**: 
  * 인프라 관리 리소스 0에 수렴. 
  * DB 변경사항 발생 시 클라이언트단에서 간편하게 리스너 콜백을 등록하여 선언적으로 화면 렌더링이 가능하여 프론트엔드 코드 복잡도 감소.
* **포기한 것 (Cons)**: 
  * 관계형 DB(RDBMS)의 강력한 트랜잭션, 복잡한 다중 조인(Join) 쿼리 기능 포기. (다만 본 서비스의 데이터 구조는 단순 공간 매칭용 key-value 형태에 가까워 NoSQL 문서 지향형인 RTDB로 충분히 대체 가능)
...
* **얻은 것 (Pros)**: 
  * 라즈베리파이 센서 노드의 CPU/메모리 부하 및 무선 네트워크 대역폭 사용량 대폭 경감.
  * 인증 토큰 갱신 등의 비즈니스 로직을 노드에서 분리하여 엣지 하드웨어 코드의 단순함과 수명 유지.
* **포기한 것 (Cons)**: 
  * 로컬 MQTT 브로커 서비스(Mosquitto 등) 및 Python Bridge 중계 스크립트를 서버단에 추가 설치/운용해야 하므로 관리 포인트 증가.
...
* **얻은 것 (Pros)**: 
  * 모션 오판으로 인해 혼잡실을 비어있다고 안내하여 이용자를 헛걸음하게 하던 피드백 오류를 완전히 방해 및 해결.
  * 실제 공부 환경에서의 추천 신뢰도 극대화.
* **포기한 것 (Cons)**: 
  * 스마트 기기를 모두 꺼두거나 소지하지 않은 소수 사용자에 대한 수치 오차 존재 가능성. (다만 PIR 타이머 쿨다운 보정을 융합 적용하여 실제 환경에서 오차를 최소화함)

### 2.4 프로그래밍 언어 및 기술 스택 선정 근거
프로젝트 각 레이어(엣지 노드, 미들웨어, 프론트엔드)에 최적의 성능과 유지보수성을 보장하기 위해 선정한 언어적 기술 스택 판단 근거입니다.

#### ① 엣지 임베디드 영역: C++17
* **대안 언어**: `Python` (RPi.GPIO 라이브러리를 통한 빠른 개발), `Go` (쉽고 빠른 채널 기반 동시성 제어)
* **채택 사유 및 구현 기능**: 라즈베리파이 3 하드웨어 단에서의 실시간 센서 폴링, 오디오 신호 샘플링(ALSA) 및 융합 판정 로직 구현.
* **선택 기준 및 장점**: 
  * 오디오 신호 실시간 샘플링(44.1kHz) 및 무선 HCI 소켓 스캔 시 발생하는 시스템 오버헤드와 가비지 컬렉터(GC)로 인한 미세 실행 지연 배제.
  * 리소스가 제한된 싱글 보드 컴퓨터(SBC) 하드웨어에 밀착하여 결정론적(Deterministic) 저지연 처리를 실행하기 위함.
* **얻은 것 (Pros)**: CPU/메모리 사용률 최소화, GC 지연이 없는 실시간 센싱 주기 수립, 시스템 저수준 API(ALSA, BlueZ)의 완벽한 제어.
* **포기한 것 (Cons)**: 메모리 수동 관리 및 동기화 코드로 인한 높은 코드 복잡도, 크로스 컴파일(Cross-compilation) 개발 환경 구축의 오버헤드.

#### ② 미들웨어 브릿지 영역: Python 3
* **대안 언어**: `C++` (엣지 노드에 종속시켜 단일 바이너리로 게이트웨이 전송까지 처리), `Node.js` (Firebase 및 MQTT와 네이티브 연동)
* **채택 사유 및 구현 기능**: MQTT 토픽을 구독(Subscribe)하고 실시간으로 Firebase RTDB에 정제된 JSON 데이터를 PUT 전송하는 미들웨어 게이트웨이 구현.
* **선택 기준 및 장점**: 
  * `paho-mqtt`, `requests`, `firebase-admin` 등 클라우드 연동에 필요한 라이브러리 생태계가 풍부하여 네트워크 병목 구간에 대한 안전 장치 구성이 용이함.
  * 데이터 규격 맵핑 및 예외 처리 비즈니스 로직 수정이 잦은 영역이므로 C++ 대비 개발 속도를 가속하기 위함.
* **얻은 것 (Pros)**: 게이트웨이 비즈니스 로직의 신속한 디버깅 및 개발 시간 단축, 클라우드 연동 모듈의 높은 가독성과 유지보수성.
* **포기한 것 (Cons)**: C++에 비해 실행 속도 및 메모리 효율성이 떨어지나, 1초 주기의 라이브 브릿지 데이터 트래픽 환경에서는 오버헤드가 극히 미미하여 문제없음.

#### ③ 프론트엔드 대시보드 영역: HTML5 / Vanilla JavaScript
* **대안 기술**: `React`, `Next.js` (모던 프론트엔드 프레임워크 기반 컴포넌트화)
* **채택 사유 및 구현 기능**: Firebase 실시간 웹소켓 데이터를 받아 1초마다 공간 추천 스코어를 재정렬하고 부드러운 순위 변화 트랜지션 애니메이션을 실행하는 화면 렌더링.
* **선택 기준 및 장점**: 
  * 빌드 및 컴파일(Babel, Webpack 등) 환경 구축 없이 즉시 브라우저에서 실행 가능한 무(Zero)설치 데모 환경 구성.
  * 가벼운 실시간 웹소켓 이벤트 렌더링을 위해 무거운 프레임워크 라이브러리 로딩 오버헤드를 배제하기 위함.
* **얻은 것 (Pros)**: 빌드 시간 0, 가벼운 브라우저 로딩 속도, 순수 DOM 제어 및 CSS 트랜지션을 이용한 렌더링 튜닝의 자유도 극대화.
* **포기한 것 (Cons)**: 컴포넌트 재사용성이 떨어지고, 복잡한 상태 관리와 페이지 라우팅 확장의 한계.
